...
import com.app.measurementservice.enums.OperationType;
import com.app.measurementservice.exception.QuantityMeasurementException;
import com.app.measurementservice.measurement.IMeasureable;
import com.app.measurementservice.measurement.LengthUnit;
import com.app.measurementservice.measurement.Quantity;
import com.app.measurementservice.measurement.TemperatureUnit;
import com.app.measurementservice.measurement.VolumeUnit;
import com.app.measurementservice.measurement.WeightUnit;
...
public class QuantityMeasurementServiceImpl implements IQuantityMeasurementService {
     ...
     * 4. Return result DTO
     */
    @CacheEvict(value = "allHistory", allEntries = true)
    @Override
    ...
    private double parseResultValue(String result) {
    ...
    private String parseResultUnit(String result) {
        if (result == null || result.isBlank()) {
            return "";
        }
    ...
    @Cacheable("allHistory")
    @Override
    public List<QuantityMeasurementDTO> getAllHistory() {
        return repository.findAll()
                .stream()
                .map(entity -> new QuantityMeasurementDTO(
                        parseResultValue(entity.getResult()),
                        parseResultUnit(entity.getResult()),
                        entity.getOperation(),
    ...
    public List<QuantityMeasurementDTO> getHistoryByOperation(String operation) {
        OperationType opType = OperationType.valueOf(operation.toUpperCase());

        return repository.findByOperation(opType)
                .stream()
                .map(entity -> new QuantityMeasurementDTO(
                        parseResultValue(entity.getResult()),
                        parseResultUnit(entity.getResult()),
                        entity.getOperation(),
                        "Filtered history fetched successfully"
                ))
                .toList();
    }
